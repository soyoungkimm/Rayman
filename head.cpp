...
int n_obj; // land 개수 (object 구조체 관련 변수)
...
void glutLeaveMainLoop()
{
    ...
    free(back.imageData);
    for (i = 0; i < sizeof(stand); i++) free(stand[i].imageData);
    for (i = 0; i < sizeof(walk); i++) free(walk[i].imageData);
    for (i = 0; i < sizeof(craw); i++) free(craw[i].imageData);
    for (i = 0; i < sizeof(jump); i++) free(jump[i].imageData);
    for (i = 0; i < sizeof(land); i++) free(land[i].imageData);
}
